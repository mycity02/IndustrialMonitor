...
│   ├── ViewModels/FunctionUC/          趋势 / 报警 / 报表 / 配置视图模型
│   ├── ViewModels/DialogWin/           趋势配置弹窗视图模型
│   └── Logger/                         NLog 日志封装
...
├── IndustrialMonitor.WinService/       Windows 后台轮询服务（TCP 订阅 / 心跳 / 实时读写）
...
└── IndustrialMonitor.Helper/           通用帮助类（MD5、密码框附加属性、DataGrid 动态列绑定等）
```
...
- [x] 趋势图表配置、坐标轴编辑、预警区间与设备变量选择
- [x] 趋势配置持久化与图表图片导出
- [x] 报警记录查询、分页、处理与 Excel 导出
- [x] 监控记录统计报表、动态列选择与 Excel 导出
- [x] 监控变量配置与用户增删、密码重置

### 运行与维护

- [x] 监控记录达到 200 条后批量入库，应用退出时刷新剩余记录
- [x] NLog 日志服务与配置文件
- [x] Windows Service 后台通信项目（TCP 客户端登记、订阅、心跳、实时读取与写入）
...
# 可选：运行后台通信服务（默认监听 8899 端口）
dotnet run --project IndustrialMonitor.WinService
```
