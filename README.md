...
本项目是一个用于学习工业上位机开发的 WPF 应用程序。系统采用 Prism + 模块化分层架构，已构建出较为完整的监控与工艺链路编辑能力：登录认证、主界面导航、实时数据面板、工业组件库、工艺链路可视化编辑、变量报警配置以及基于 MySQL 的数据持久化。
...
- [ ] 报表模块数据导出
- [ ] 配置模块参数设置
- [ ] 设备实时通信模块（串口 / TCP / Modbus / S7 / FINS / MC）
- [ ] SignalR 实时消息推送
